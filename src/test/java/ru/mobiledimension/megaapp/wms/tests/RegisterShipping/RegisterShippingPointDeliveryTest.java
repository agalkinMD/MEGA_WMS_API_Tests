...
@Feature("Запрос на оформление доставки в пункт выдачи")
public class RegisterShippingPointDeliveryTest extends BaseRequest {
...
//    TODO: избавиться от duplicated-кода в @BeforeClass
    ...
    public void getResponse() {
        response =
            given()
                .spec(registerShippingPointDelivery_requestSpec)
            .when()
    ...
    public void RegisterShippingPointDelivery_validateBarcodeList() {
        if (isEmpty(getVerifiedBarcodeListFile())) {
            ...
            Assert.assertTrue(isEqualContentInActualAndVerifiedFiles(getActualBarcodeListFile(), getVerifiedBarcodeListFile()),
                    "Content in API response and \"PurchasesList_verified\" file doesn't match!");
        }
    }
...
//    TODO: подумать над оптимизацией проверки licensePlate
    ...
    @Severity(SeverityLevel.NORMAL)
    public void RegisterShippingPointDelivery_validateDeliveryParameters() {
        for (Purchase purchase : responseBody.getPurchases()) {
            Assert.assertEquals(purchase.getCellType(),Integer.valueOf(1));
            Assert.assertEquals(purchase.getCellID(), "8322");
    ...
    @Severity(SeverityLevel.NORMAL)
    public void RegisterShippingPointDelivery_validateDeliveryTimer() {
        for (Purchase purchase : responseBody.getPurchases()) {
...
//    TODO: избавиться от duplicated-кода в @AfterClass
    ...
    public void prepareForNextAPIMethodCall() {
        ...
        RegisterShippingCancelTest registerShippingCancelTest = new RegisterShippingCancelTest(registerShippingCancelPointDelivery_requestSpec);
